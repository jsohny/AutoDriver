...
test_total_rmse = np.zeros((9, 1))
lookback_vals = np.arange(2, 11)
for lookback_val in lookback_vals:
...
# then use these to create predictions and save to file.
  ...
  for idx, tup in enumerate(test_data):
      ...
      model_x = TimeSeriesRegression(lookback=lookback_val)
      model_x.create(agent_coords_x)
      model_y = TimeSeriesRegression(lookback=lookback_val)
...
plt.plot(lookback_vals, test_total_rmse[:,0], label="testing")

plt.xlabel("Lookback Value")
plt.ylabel("Total RMSE")
plt.legend()
fname = os.path.join("..", "figs", "lookback_rmse_plot.pdf")
plt.savefig(fname)